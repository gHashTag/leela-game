import notifee, { Event, EventType } from '@notifee/react-native'
import { navRef } from 'src/constants'
import { getUid } from 'src/screens/helper'
import { MessagingStore, PostStore } from 'src/store'
import {
  cancel,
  updateAndroidBadgeCount,
} from 'src/utils/notifications/NotificationHelper'
import { updateAndroidCommentNotificationGroup } from 'src/utils/notifications/replyNotification'

export const replyActionHandler = async ({ type, detail }: Event) => {
  const { pressAction, notification, input } = detail
  const { commentOwner, commentId, postId }: any = notification?.data || {}

  switch (type) {
    case EventType.ACTION_PRESS:
      switch (pressAction?.id) {
        case 'reply':
          if (commentOwner && commentId && input) {
            await cancel({ notification, isInput: true })
            await PostStore.replyComment({
              text: input,
              commentOwner,
              commentId,
              postId,
            })
          }
          break
      }
      break

    case EventType.PRESS:
      await notifee.decrementBadgeCount()
      updateAndroidBadgeCount({ type: 'decrement' })

      if (navRef && navRef.isReady()) {
        if (getUid()) {
          navRef.navigate('DETAIL_POST_SCREEN', { postId: postId })
        }
      } else {
        const path = `/reply_detail/${postId}`
        MessagingStore.path = path
      }
      break

    case EventType.DISMISSED:
      updateAndroidBadgeCount({ type: 'decrement' })
      await notifee.decrementBadgeCount()
      break
  }
  updateAndroidCommentNotificationGroup()
}
